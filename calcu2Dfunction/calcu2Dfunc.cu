...
__global__ void calcu2DArray(cuDoubleComplex* dev_sig, cuDoubleComplex* dev_window_h,
    ...
    for (int i = 0; i < 8; i++)
    {
      for (int k = 0; k < minor_dim; k++)
      {
        for (int j = 0; j < N; j++)
        {
          sum =cuCadd(sum, cuCmul(dev_sig[i * N + j], cuCmul(dev_window_h[j], cuCmul(dev_exp_val[k * N + j], dev_x_ref[global_tid_y * N + j]))));
        }
        dev_rd_array[i * major_dim * minor_dim + k * major_dim + global_tid_y] = sum;
      }
    } 
}
